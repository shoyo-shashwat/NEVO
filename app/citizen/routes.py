...
from app.auth.session import current_actor_id, current_role, current_identity_fields, current_country_code
from app.auth.rbac import current_citizen_account
from app.auth.security import generate_complaint_id, CURRENT_CONSENT_VERSION
...
@citizen_bp.route("/regions/<region_id>/children")
def region_children(region_id):
    """
    JSON endpoint backing the cascading state -> district manual-location
    picker on the report form (Phase 0 / product spec §2 "manual location
    selection"). Real data from AdministrativeRegion — not a static list.
    """
    children = (
        AdministrativeRegion.query
        .filter_by(parent_region_id=region_id)
        .order_by(AdministrativeRegion.name)
        .all()
    )
    return jsonify([{"id": r.id, "name": r.name} for r in children])
...
def report_flow():
    ...
    if request.method == "GET":
        ...
        return render_template(
            ...
            needs_consent=citizen_account is None,
        )
    ...
    # Manual location selection — an explicit district-level region chosen
    # from the state->district dropdowns, distinct from the free-text
    # location_hint used for AI extraction and from GPS. When present, this
    # is authoritative and skips the fuzzy ILIKE match entirely.
    manual_region_id = (request.form.get("manual_region_id") or "").strip() or None
    ...
    # Consent — required every time for anonymous submission (no account to
    # hold it); already on file at signup for authenticated citizens, so it
    # is not re-asked (Phase 0 §4 / product spec §2 "Consent").
    if citizen_account is None and request.form.get("consent") != "on":
        flash("Please confirm consent to submit a report.", "error")
        states = (
            AdministrativeRegion.query
            .filter_by(country_id=country_id, level="state_province")
            .order_by(AdministrativeRegion.name)
        ...
        return render_template(
            "citizen/report_flow.html",
            states=states, categories=Category.query.all(), needs_consent=True,
        )
    ...
    region_id = manual_region_id  # authoritative when the citizen picked one explicitly
    if not region_id and extracted.get("location") and country_id:
        region = (
            AdministrativeRegion.query
    ...
    # --- Draft/Submitted gate (Progress Log §13.1 / Phase 0 §6) ---
    # "Submitted" replaces the old "Unclustered" as the first post-Draft
    # state — the fuller lifecycle (Processing/Verified/Clustered/...) is
    # advanced below as the pipeline actually runs, not all at once here.
    status = "Submitted" if meta.get("complete") else "Draft"

    identity = current_identity_fields()  # exactly one of citizen_account_id / anonymous_token
    from app.models.shared import Country as _Country
    country = db.session.get(_Country, country_id) if country_id else None
    complaint_id = generate_complaint_id(country.code if country else "IN")

    report = Report(
        complaint_id=complaint_id,
        citizen_account_id=identity["citizen_account_id"],
        anonymous_token=identity["anonymous_token"],
        ...
        latitude=latitude,
        longitude=longitude,
        consent_given_at=None if citizen_account else datetime.now(timezone.utc),
    )
    db.session.add(report)
    db.session.flush()   # get report.id before commit

    # Evidence — photo, and/or additional attachments (multiple files).
    # Stored locally under app/static/uploads/evidence/ (see
    # services/evidence_storage.py). Failure here must never block report
    # submission — evidence is optional.
    uploaded_by_label = identity["citizen_account_id"] or "anonymous"
    evidence_files = [
        f for f in request.files.getlist("evidence_files") if f and f.filename
    ]
    # Back-compat single-field name still accepted (older form markup).
    single = request.files.get("evidence_photo")
    if single and single.filename:
        evidence_files.append(single)
    ...
    for evidence_file in evidence_files:
        try:
            ...
            logger.warning("Evidence upload skipped: %s", e)
        except Exception as e:
            logger.warning("Evidence upload failed, continuing without it: %s", e)
    ...
    try:
        ...
        # AI extraction + embedding succeeded — distinct pipeline milestone
        # from the citizen-facing Verification model's "StillHappening" etc.
        # votes; this "Verified" means the AI pipeline verified the report is
        # well-formed and embedded, not a community sentiment.
        report.status = "Verified"
    except Exception as embed_err:
        # Embedding or pgvector query failed — rollback and continue without
        # matching. Report stays at "Submitted"; it can still be viewed and
        # matched again later once the underlying issue is fixed.
        logger.warning("Embed/match failed, continuing without cluster match: %s", embed_err)
        db.session.rollback()
        # Re-add the report after rollback (status reverts to Submitted too)
        ...
        if action == "add_evidence" and cluster_id:
            ...
            try:
                ...
                db.session.add(Evidence(
                    type="photo",
                    url=photo_url,
                    uploaded_by=report.citizen_account_id or "anonymous",
...
def my_timeline():
    ...
    # All reports by this citizen. Anonymous submissions are intentionally
    # excluded — there is no account to attribute them to, and "never expose
    # anonymous citizen identity" means we don't try to match them here
    # either (Phase 0 design §4).
    reports = (
        Report.query
...
# ---------------------------------------------------------------------------
# Public complaint tracking — works for anonymous AND authenticated
# submissions, since a citizen may submit anonymously and still want to
# check status later using the complaint ID they were given (Phase 0 §5).
# Shows status only — never citizen identity (product spec §15).
# ---------------------------------------------------------------------------
...
def track_complaint():
    ...
    if request.method == "POST":
        searched = True
        complaint_id = (request.form.get("complaint_id") or "").strip().upper()
        report = Report.query.filter_by(complaint_id=complaint_id).first()
        if not report:
            flash("No report found with that complaint ID. Check it and try again.", "error")

    category_name = None
    if report and report.category_id:
        cat = db.session.get(Category, report.category_id)
        category_name = cat.name if cat else None

    return render_template(
        "citizen/track_complaint.html",
        report=report, searched=searched, category_name=category_name,
    )


@citizen_bp.route("/report/<report_id>/close", methods=["POST"])
def close_report(report_id):
    """
    A citizen (or anonymous submitter, via their own persistent token)
    confirms their resolved report can be closed. Only reachable once the
    report has actually reached Resolved — closing is a real, narrow action,
    not a generic delete/dismiss.
    """
    report = Report.query.get_or_404(report_id)
    identity = current_identity_fields()
    owns_report = (
        (identity["citizen_account_id"] and identity["citizen_account_id"] == report.citizen_account_id)
        or (identity["anonymous_token"] and identity["anonymous_token"] == report.anonymous_token)
    )
    if not owns_report:
        abort(403)
    if report.status != "Resolved":
        flash("This report isn't marked Resolved yet, so it can't be closed.", "error")
    else:
        report.status = "Closed"
        db.session.commit()
        flash("Thanks for confirming — this report is now closed.", "success")

    if identity["citizen_account_id"]:
        return redirect(url_for("citizen.my_timeline"))
...
def _country_id_from_session() -> str:
    """Map the visitor's country (account or browse preference) to a Country ID."""
    from app.models.shared import Country
    code = current_country_code() or "IN"
...
def _guard_against_spam_and_duplicates(raw_text: str):
    ...
    to proceed. Applies to anonymous submitters too — each gets a persistent
    anonymous_token (Phase 0 §4), so "the same citizen" is well-defined even
    without an account.
    """
    identity = current_identity_fields()
    actor_id = identity["citizen_account_id"] or identity["anonymous_token"]
    ...
    recent = (
        Report.query
        .filter(
            db.or_(Report.citizen_account_id == actor_id, Report.anonymous_token == actor_id),
