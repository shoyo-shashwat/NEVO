...
class MpladsLedger(db.Model):
    """
    One row per (seat, financial year): what the MPLADS entitlement looks like
    "as reported by <source> on <date>" -- deliberately NOT a single unified
    truth (allocated / released / sanctioned / spent come from sources with
    different lag, so they are stored as reported and never reconciled here).

    Derived values (e.g. recommended-but-not-yet-sanctioned) are intentionally not columns -- computed per request in
    app/services/mplads_ledger.py, same rule as every analytical value in
    this app. An MP recommends works; the district authority sanctions and
    implements them, so nothing here is something NEVO allocates or disburses.
    """
    __tablename__ = "mplads_ledgers"
    __table_args__ = (
        db.UniqueConstraint("region_id", "period_label", name="uq_mplads_ledger_region_period"),
    )

    id = db.Column(db.String(36), primary_key=True, default=_uuid)
    region_id = db.Column(db.String(36), db.ForeignKey("administrative_regions.id"), nullable=False)
    seat_label = db.Column(db.String(200), nullable=False)        # e.g. "New Delhi Parliamentary Constituency"
    # The official portal publishes cumulative figures for a Lok Sabha tenure,
    # not per financial year -- e.g. "18th Lok Sabha (2024-29), term to date".
    period_label = db.Column(db.String(60), nullable=False)
    term_end = db.Column(db.Date, nullable=True)

    # Rupees, Numeric(18,2) -- same convention as GovernmentInvestment.budget.
    # No "released" column: since April 2025 MPLADS pays vendors directly, so the
    # official portal publishes no release-to-district figure.
    allocated = db.Column(db.Numeric(18, 2), nullable=True)
    recommended = db.Column(db.Numeric(18, 2), nullable=True)   # works the MP has recommended
    sanctioned = db.Column(db.Numeric(18, 2), nullable=True)    # of those, sanctioned by the district
    spent = db.Column(db.Numeric(18, 2), nullable=True)
    works_recommended = db.Column(db.Integer, nullable=True)
    works_sanctioned = db.Column(db.Integer, nullable=True)
    works_completed = db.Column(db.Integer, nullable=True)
    notes = db.Column(db.Text, nullable=True)                   # caveats the source itself shows

    source = db.Column(db.String(300), nullable=True)
    source_url = db.Column(db.Text, nullable=True)
    source_last_updated = db.Column(db.Date, nullable=True)
    platform_last_synced = db.Column(db.DateTime(timezone=True), nullable=True)
    method = db.Column(db.String(40), nullable=True)              # "manual" | "api" | "seed"
    ...
    def __repr__(self):
        return f"<MpladsLedger region={self.region_id} period={self.period_label}>"
