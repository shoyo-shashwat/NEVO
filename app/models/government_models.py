...
class GovernmentDecision(db.Model):
    ...
    # decided_by_id kept as a plain string for historical rows created under
    # the old demo-actor system; new decisions also populate
    # decided_by_account_id (a real FK) alongside it. decided_by_role widened
    # to the six production roles (Phase 0) — "mp"/"planning_officer" remain
    # valid enum values only so existing seeded rows don't need rewriting.
...
class Project(db.Model):
    ...
    # Ownership — needed for the "your projects in progress" dashboard
    # section (design mapping, Government §7/§12). NULL = unassigned.
