...
class DemandCluster(db.Model):
    ...
    def unique_contributors(self) -> int:
        """
        Count of distinct contributors across this cluster's Contributions —
        the anti-manipulation mechanism (§5.2.2): one citizen submitting 10
        reports still counts as 1.

        Phase 0 fix: counts COALESCE(citizen_account_id, anonymous_token)
        instead of the old citizen_id string. The old column collapsed every
        anonymous submitter platform-wide into the single literal "anon",
        which meant COUNT(DISTINCT citizen_id) silently undercounted any
        cluster with more than one anonymous contributor. anonymous_token is
        a per-citizen persistent cookie value, so distinct anonymous
        contributors now count as distinct (see Report.identity_key).
        """
        from app.models.citizen_models import Contribution  # local import avoids circularity
        identity = func.coalesce(Contribution.citizen_account_id, Contribution.anonymous_token)
        return (
