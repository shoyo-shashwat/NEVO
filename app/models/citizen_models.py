...
class Report(db.Model):
    ...
    # Human-readable reference the citizen can quote/track by — independent
    # of the internal UUID primary key (Phase 0 §5). Generated once at
    # creation, never regenerated.
    complaint_id = db.Column(db.String(40), unique=True, nullable=False, index=True)

    # Identity & scope.
    # Exactly one of citizen_account_id / anonymous_token is set (Phase 0 §4)
    # — never both, never neither. citizen_id is kept as a read-only derived
    # property below for code that doesn't care which kind of identity this is.
    ...
    # Lifecycle status — expanded per Phase 0 §6. "Unclustered" is kept as a
    # legacy value so existing rows migrate without rewriting; new reports
    # use the full lifecycle instead of stopping at Unclustered.
    status = db.Column(
        db.Enum(
            "Draft", "Unclustered", "Submitted", "Processing", "Verified",
            "Clustered", "UnderReview", "Actioned", "Resolved", "Closed",
    ...
    # Consent captured at submission time — required for authenticated
    # citizens (captured once at signup instead); for anonymous submissions
    # this is per-report since there is no account to hold it.
    ...
    def identity_key(self) -> str:
        """
        Single value to group/count by regardless of whether this report is
        authenticated or anonymous — never expose this outside the backend
        (Phase 0 §4 — anonymous_token carries no PII but is still only an
        internal counting key, not a public identifier).
        """
        return self.citizen_account_id or self.anonymous_token or ""

    def __repr__(self):
        return f"<Report {self.complaint_id} status={self.status} channel={self.channel}>"
...
class Contribution(db.Model):
    ...
    # citizen_id is replaced by citizen_account_id / anonymous_token
    # (Phase 0 §4) — see Report.identity_key for the counting rule.
    report_id = db.Column(
...
class Verification(db.Model):
    ...
    def __repr__(self):
        return (
            f"<Verification citizen={self.identity_key} "
