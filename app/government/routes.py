...
from app.models.mplads_models import MpladsStateSummary, MpladsLedger
...
from app.services import gap_assessment, investment_alignment, priority_scoring, development_insight, alignment_analytics, mplads_permissibility, mplads_ledger
...
def dashboard():
    ...
    # Filter bar (folded in from the old Demand Intelligence page). Any filter switches the
    # queue tabs for the full matching list, not just the top 20.
    sector_filter = request.args.get("sector") or None
    alignment_filter = request.args.get("alignment") or None
    status_filter = request.args.get("status") or None
    filtered_cards = None
    if sector_filter or alignment_filter or status_filter:
        filtered_cards = _filtered_cards(country_id, mp_region_ids, sector_filter, alignment_filter, status_filter)
    ...
    # One lens for the MP (the Planning Officer role was folded into MP):
    # the queue tabs on dashboard.html are just different cuts of `cards`.
    role = current_role()
    _ipq = Project.query.filter(Project.country_id == country_id, Project.status != "Completion")
    if mp_region_ids:
        _ipq = _ipq.filter(or_(Project.region_id.in_(mp_region_ids), Project.region_id.is_(None)))
    in_progress_projects = _ipq.order_by(Project.updated_at.desc()).limit(5).all()
    # "Needs your decision" = top priorities + awaiting sign-off, de-duplicated.
    needs_decision, _seen = [], set()
    for card in top + pending:
        if card["cluster"].id not in _seen:
            _seen.add(card["cluster"].id)
            needs_decision.append(card)
    ...
    # --- Fund ledger (MPLADS entitlement for this seat, as reported by its source) ---
    ledger = None
    if mp_region_id:
        try:
            row = (MpladsLedger.query.filter_by(region_id=mp_region_id)
                   .order_by(MpladsLedger.created_at.desc()).first())
            ledger = mplads_ledger.summarize(row, date.today())
        except ProgrammingError:      # table not migrated yet -- dashboard must still load
            db.session.rollback()
    fundable_gaps = [c for c in cards
                     if c["alignment_state"] in ("UNADDRESSED", "PARTIALLY_ADDRESSED", "EMERGING_GAP")
                     and c["verdict"]["kind"] in ("yes", "review")]

    # --- This week: real counts only (a "since last login" needs a stored last-seen; not built) ---
    from app.models.citizen_models import Report
    week_q = (db.session.query(Report.category_id, func.count(Report.id))
              .filter(Report.country_id == country_id,
                      Report.created_at >= datetime.now(timezone.utc) - timedelta(days=7)))
    if mp_region_ids:
        week_q = week_q.filter(or_(Report.region_id.in_(mp_region_ids), Report.region_id.is_(None)))
    week_counts = week_q.group_by(Report.category_id).all()
    week = None
    if week_counts:
        top_cat_id, top_n = max(week_counts, key=lambda r: r[1])
        top_cat = db.session.get(Category, top_cat_id) if top_cat_id else None
        week = {"total": sum(n for _, n in week_counts),
                "top_name": top_cat.name if top_cat else None, "top_n": top_n}

    # --- Did it work? Completed projects vs what citizens say now (money joins in once works are linked) ---
    results = []
    done_q = Project.query.filter(Project.country_id == country_id, Project.status == "Completion")
    if mp_region_ids:
        done_q = done_q.filter(or_(Project.region_id.in_(mp_region_ids), Project.region_id.is_(None)))
    for p in done_q.order_by(Project.updated_at.desc()).limit(5).all():
        cl = db.session.get(DemandCluster, p.linked_demand_cluster_id) if p.linked_demand_cluster_id else None
        oc = Outcome.query.filter_by(project_id=p.id).first()
        sent = cl.community_sentiment if cl else None
        results.append({
            "project": p, "cluster": cl,
            "still_pct": sent["still_affected_pct"] if sent and sent["votes"] else None,
            "votes": sent["votes"] if sent else 0,
            "verified_impact": oc.impact_percent if oc and oc.status == "Verified" else None,
        })

    return render_template(
        "government/dashboard.html",
        role=role,
        needs_cards=needs_decision[:8],
        ...
        ledger=ledger, fundable_gaps=len(fundable_gaps), week=week, results=results,
        categories=Category.query.all(),
        filtered_cards=filtered_cards,
        sector_filter=sector_filter, alignment_filter=alignment_filter, status_filter=status_filter,
        mplads=mplads,
    )
...
def _filtered_cards(country_id, mp_region_ids, sector=None, alignment=None, status=None):
    """The full, filterable list of demands (was the Demand Intelligence page)."""
    query = DemandCluster.query.filter(DemandCluster.country_id == country_id)
    if sector:
        query = query.filter(DemandCluster.category_id == sector)
    if status:
        query = query.filter(DemandCluster.active_status == status)
    else:
        query = query.filter(DemandCluster.active_status.in_(["Active", "UnderGovernmentReview"]))
    # region_ids is a plain JSON column (no `@>`), so scope is applied in Python.
    ...
    cards = [_build_cluster_card(c) for c in clusters]
    # alignment_state is computed fresh, not a stored column, so it filters after card-building.
    if alignment:
        cards = [c for c in cards if c["alignment_state"] == alignment]
    return cards
...
def demand_intelligence():
    """Folded into the dashboard's filter bar; kept so old links and bookmarks still land somewhere useful."""
    return redirect(url_for("government.dashboard", **request.args))
...
def national_overview():
    ...
    return render_template(
        ...
        role=current_role(),   # _cluster_card.html picks its action by role
    )
...
def citizen_voice():
    ...
    recent_reports = query.order_by(Report.created_at.desc()).limit(60).all()
...
def evidence_detail(cluster_id):
    ...
    sample_reports = _sample_reports(cluster_id)
...
def decision_workspace(cluster_id):
    ...
    if request.method == "POST":
        ...
        valid_types_mp = ("Prioritize", "Defer", "Deprioritize", "NeedsValidation",
                          "Redirected")
        if decision_type not in valid_types_mp:
        ...
        # Back to where the decision was started from. This screen is now
        # entered from the dashboard rather than from the evidence card, so
        # returning to the evidence card would be a detour to a page the
        # officer deliberately skipped; the flash confirms the write and the
        # dashboard card shows the updated status.
        return redirect(url_for("government.dashboard"))

    # GET — collect available projects for the "Link project" dropdown
    _, _scope_ids, _ = _government_scope(cluster.country_id)
    _pq = Project.query.filter_by(country_id=cluster.country_id)
    if _scope_ids:
        _pq = _pq.filter(or_(Project.region_id.in_(_scope_ids), Project.region_id.is_(None)))
    projects = _pq.all()
    category = db.session.get(Category, cluster.category_id)

    # The evidence behind the decision, computed with the same services
    # evidence_detail() uses so the two screens can never disagree. This
    # screen is now reachable directly from the dashboard, so the reason
    # textarea must never sit next to nothing to read: whoever authors
    # GovernmentDecision.reason sees the citizen voices and the alignment
    # reasoning on the same page they write it on.
    gap = gap_assessment.calculate(cluster_id)
    alignment = investment_alignment.calculate(cluster_id, gap_assessment=gap)
    dominant_sev = _dominant_severity(cluster_id)
    s1 = priority_scoring.stage1_confidence(gap, alignment)
    ...
    return render_template(
        ...
        priority=s2,
        confidence=s1,
        sample_reports=_sample_reports(cluster_id, limit=2),
        verdict=mplads_permissibility.verdict(alignment.state, category.code if category else ""),
    )
...
def _build_cluster_card(c: DemandCluster) -> dict:
    ...
    return {
        ...
        "confidence": s1.confidence,
        "alignment_state": alignment.state,
        "verdict": mplads_permissibility.verdict(alignment.state, cat.code if cat else ""),
    }


def _sample_reports(cluster_id: str, limit: int = 3):
    """
    A few representative citizen quotes for a cluster, in English.
    ...
    see app/models/citizen_models.py). problem_summary_en is the "common
    format" structured field groq_client.extract_report_fields() produces
    for exactly this, so an officer gets real readable context regardless
    of the underlying reports' language.

    Fetch a larger pool and dedupe by text in Python (not a DB-level
    DISTINCT, to stay portable) -- otherwise several citizens reporting the
    same problem in near-identical words (common for a templated demo, and
    plausible for real citizens describing the same visible issue) shows as
    the same quote repeated 3 times, which reads as a bug even though each
    row is a real report.

    Shared by evidence_detail() and decision_workspace() so the same three
    voices back the decision as back the evidence page.
    """
    from app.models.citizen_models import Report
    ...
    candidates = (
        ...
        .filter(Contribution.demand_cluster_id == cluster_id,
                Report.problem_summary_en.isnot(None))
    ...
    out, seen = [], set()
    for r in candidates:
        if r.problem_summary_en in seen:
            continue
        seen.add(r.problem_summary_en)
        out.append(r)
        if len(out) == limit:
            break
    return out
