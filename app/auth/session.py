    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            if session.get("role") not in roles:
                flash("Please select a demo account to continue.", "info")
                # Route 4 UI fix: send unauthenticated visitors straight to the
                # account picker, not the marketing landing page — this was
                # part of the "reduction is not right" report (a citizen/anon
                # visitor hitting a gov-only route landed on "/" with no
                # obvious next step other than scrolling to find the switcher,
                # which no longer even lives there).
