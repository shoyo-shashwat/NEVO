...
from app.services import mplads_ledger as ledger_rules
...
def seed_mplads_ledger():
    """
    Load ONE seat's MPLADS ledger from seed/mplads_ledger.json (copy the .example
    and fill it from the source). Figures are never invented: no file means no
    row, and the dashboard says so. validate() rejects impossible figures here,
    not in a template. Idempotent upsert by (region, period_label).
    """
    import json
    path = os.path.join(os.path.dirname(__file__), "mplads_ledger.json")
    if not os.path.exists(path):
        print("  MPLADS ledger: no seed/mplads_ledger.json -- skipped (dashboard shows 'not loaded')")
        return
    d = json.load(open(path, encoding="utf-8"))
    region_id = {cfg["code"]: cfg["state_id"] for cfg in STATE_EXPANSION}[d["region_code"]]
    ledger_rules.validate(d["allocated"], d["recommended"], d["sanctioned"], d["spent"])
    row = MpladsLedger.query.filter_by(region_id=region_id, period_label=d["period_label"]).first()         or MpladsLedger(region_id=region_id, period_label=d["period_label"])
    row.seat_label = d["seat_label"]
    for k in ("allocated", "recommended", "sanctioned", "spent", "works_recommended", "works_sanctioned",
              "works_completed", "source", "source_url", "method", "notes"):
        setattr(row, k, d.get(k))
    row.term_end = date.fromisoformat(d["term_end"]) if d.get("term_end") else None
    row.source_last_updated = date.fromisoformat(d["read_on"])   # OUR retrieval date; sources print none
    row.platform_last_synced = _now()
    db.session.add(row)
    db.session.commit()
    print(f"  MPLADS ledger ({d['seat_label']}, {d['period_label']}): OK")
